 ...
 int cashback()
{
	...
	// Open peripherals
	    ...
	    char saleAmount[100];
	    char cashbackAmount[256];
	    char amountmessage[256];
	    char cashbackmessage[256];
	    ulong message = 0;
	    int amountSale = 0;
	    int amountCashback = 0;
		...
		amountResult = GL_Dialog_Amount(gGoalGraphicLibInstance, "Cashback", "Enter Amount", saleMasc, cashbackAmount, sizeof(saleAmount),currency, GL_ALIGN_BOTTOM_RIGHT, 6* 1000);

		memset(cashbackmessage, 0, sizeof cashbackmessage);
		strcat(cashbackmessage, cashbackAmount);


//     Type cast amount to enable validation
	   sscanf(saleAmount, "%d", &amountSale);
	   amountSale = amountSale;

	   sscanf(cashbackAmount, "%d", &amountCashback);
	   amountCashback = amountCashback;

	   if(amountCashback > amountSale){
	   message = GL_Dialog_Message(gGoalGraphicLibInstance, NULL, "Please enter valid cashback Amount", GL_ICON_NONE, GL_BUTTON_ALL, 2*1000);
	    ...
	    GL_Dialog_Message(gGoalGraphicLibInstance, NULL, "Please Insert or Swipe Card", GL_ICON_INFORMATION, GL_BUTTON_NONE, 0);
	    	...
	    	// Wait for magnetic card
	    		...
	    		// Retrieve and analyze ISO2
					...
					xPrint = GL_Print_Create    (xLayout);
					GL_Widget_SetText      (xPrint, "Cashback");
					GL_Widget_SetItem      (xPrint, 0, xline++);
					GL_Widget_SetFontScale (xPrint, GL_SCALE_LARGE);
					GL_Widget_SetBackAlign (xPrint, GL_ALIGN_CENTER);
					GL_Widget_SetWrap      (xPrint, TRUE);
					...
					xPrint = GL_Print_Create    (xLayout);
					GL_Widget_SetText      (xPrint, cashbackmessage);
					GL_Widget_SetItem      (xPrint, 0, xline++);
					GL_Widget_SetFontScale (xPrint, GL_SCALE_LARGE);
					GL_Widget_SetBackAlign (xPrint, GL_ALIGN_CENTER);
					GL_Widget_SetWrap      (xPrint, TRUE);
