...
void Smarts(void)
{
    ...
    // ********************
	hScreen = GoalCreateScreen(gGoalGraphicLibInstance, txSmart, NUMBER_OF_LINES(txSmart), GL_ENCODING_UTF8);
	...
	// Display ATR
	// ***********
	hScreen = GoalCreateScreen(gGoalGraphicLibInstance, txCmd, NUMBER_OF_LINES(txCmd), GL_ENCODING_UTF8);
	CHECK(hScreen!=NULL, lblKO);                              // Create new screen and clear it
	iRet = GoalClrScreen(hScreen, GL_COLOR_BLACK, 0, false);
	CHECK(iRet>=0, lblKO);

	iRet = GoalDspLine(hScreen, 0, "Card Details:", &txCmd[0], 0, false);
	CHECK(iRet>=0, lblKO);
	...
	iRet = GoalPrnLine(xDocument, p++, "Card Details", &xLine);
	CHECK(iRet>=0, lblKO);

	xLine = xPrinter;
	iRet = GoalPrnLine(xDocument, p++, "Details:", &xLine);       // ATR info
	CHECK(iRet>=0, lblKO);
    ...
    if (hSmc) {
		...
		case CAM_PRESENT:                                     // *** Card present ***
			if (iRet==0) {
				...
				GL_Dialog_Message(gGoalGraphicLibInstance, NULL, tcDisplay, GL_ICON_ERROR, GL_BUTTON_NONE, 0);
			}
